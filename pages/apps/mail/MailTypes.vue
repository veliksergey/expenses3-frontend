...
const menuItems = ref([
    {
        label: 'Archive',
        icon: 'pi pi-file',
        command: () => onSelectedMailsAction('archive')
    },
    {
        label: 'Spam',
        icon: 'pi pi-ban',
        command: () => onSelectedMailsAction('spam')
    },
    {
        label: 'Delete',
        icon: 'pi pi-trash',
        command: () => onSelectedMailsAction('trash')
    }
]);
...
<template>
    <DataTable ref="mailTable" :value="mails" v-model:selection="selectedMails" v-model:filters="filterTable" :rows="10" paginator :rowsPerPageOptions="[10, 20, 30]" dataKey="id" rowHover :globalFilterFields="['from', 'to', 'title', 'message']">
                ...
                <td v-if="!data.trash && !data.spam" class="w-4rem">
                    <span class="cursor-pointer" @click="changeMailType('starred', data)">
                        <i
                            ...
                            :class="{
                                'pi-star-fill': data.starred,
                                'pi-star': !data.starred
                            }"
                        ></i>
                    </span>
                </td>
                <td v-if="!data.trash && !data.spam" class="w-4rem">
                    <span class="cursor-pointer ml-3" @click="changeMailType('important', data)">
                        <i
                            class="pi pi-fw text-xl"
                            :class="{
                                'pi-bookmark-fill': data.important,
                                'pi-bookmark': !data.important
                            }"
                        ></i>
                    </span>
                </td>
        ...
        <Column style="min-width: 4rem">
            <template #body="{ data }">
                <div @click="onNavigateToDetailPage(data.id)" :style="{ minWidth: '12rem' }" class="text-900 font-semibold cursor-pointer">
                    {{ data.from || data.to }}
                </div>
            </template>
        </Column>
